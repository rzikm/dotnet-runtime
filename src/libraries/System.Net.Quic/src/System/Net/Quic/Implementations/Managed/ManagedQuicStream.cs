...
namespace System.Net.Quic.Implementations.Managed
{
    public sealed class ManagedQuicStream : QuicStream
    {
        ...
        private Task? _readsClosed;
        public override Task ReadsClosed => _readsClosed ??= (ReceiveStream is not null ? ReceiveStream.ReceiveClosed.Task : Task.CompletedTask);
        public override int ReadTimeout { get => throw new NotSupportedException(); set => throw new NotSupportedException(); }
        private Task? _writesClosed;
        public override Task WritesClosed => _writesClosed ??= (SendStream is not null ? SendStream._writesCompleted.GetTask().AsTask() : Task.CompletedTask);
